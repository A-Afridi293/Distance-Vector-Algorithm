...
int main(int argc, char *argv[] )
{
  ...
  //DVTABLE = (struct new_vector_struct *) malloc (sizeof (struct new_vector_struct)*numberOfNodes+1);
...
int printDV(struct new_vector_struct DV[], int numberOfNodes){
  ...
  //here I should be printing the new_vector_struct since it is the distance vector table that has all my nodes within in it
// that would be DV TABLE
//since DVTABLE = malloc (sizeof (struct new_vector_struct)*numberOfNodes+1);
//This means my current distance vector table is stored in DVTABLE
...
int readNeighborsFile(struct neighbor_struct neighbors[], char *filename, int myNode){
  ...
  /* loop and read values. I don't check that i overflow the table. that is bad. */
  while (fscanf(fileptr, "%d %s %d ",
                &nodeNumber, ipaddress, &portNumber) == 3 ) {
    // format of file is FromNode ToNode Cost ipaddress
#ifdef DEBUG
    printf ("readNeighborsFile: node %d, ipaddress  %s, portNumber %d\n",
            nodeNumber, ipaddress, portNumber);
#endif
